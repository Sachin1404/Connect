...
const ProfileHeader = ({ userData, onSave, isOwnProfile }) => {
	...
	const { mutate: sendConnectionRequest } = useMutation({
		mutationFn: (username) => axiosInstance.post(`/connections/requests/${username}/send`),
		onSuccess: () => {
	...
	const { mutate: acceptRequest } = useMutation({
		mutationFn: (requestId) => axiosInstance.put(`/connections/requests/${requestId}/accept`),
		onSuccess: () => {
	...
	const { mutate: rejectRequest } = useMutation({
		mutationFn: (requestId) => axiosInstance.put(`/connections/requests/${requestId}/reject`),
		onSuccess: () => {
	...
	const { mutate: removeConnection } = useMutation({
		mutationFn: (username) => axiosInstance.delete(`/connections/${username}/remove`),
		onSuccess: () => {
	...
	const renderConnectionButton = () => {
		...
		switch (getConnectionStatus) {
				...
				return (
					<div className='flex gap-2 justify-center'>
						...
						<button
							...
							onClick={() => removeConnection(userData.username)}
						>
			...
			default:
				return (
					<button
						onClick={() => sendConnectionRequest(userData.username)}
